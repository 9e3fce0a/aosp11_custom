...
CUSTOM_FINGERPRINT=$(awk -F"=" '/ro.bootimage.build.fingerprint/{print $2}' ${BUILD_DIR}/vendor/android-prepare-vendor/${DEVICE}/$(tr '[:upper:]' '[:lower:]' <<< "${AOSP_BUILD}")/factory_imgs_data/vendor/build.prop)
